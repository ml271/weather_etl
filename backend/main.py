...
def get_summary(
    ...
    # Fetch all active alerts, sorted by severity using an inline CASE expression
    # so that the most critical alerts always appear first in the UI.
    active_alerts = (
...
def get_alerts(
    ...
    """Return weather alerts for a city, with optional filters.

    Alerts are generated by the Airflow transform task when daily forecast
    values exceed the thresholds defined in ``config/alerts_config.yaml``.

    Args:
        city: City name to query. Falls back to ``DEFAULT_CITY`` when omitted.
        active_only: When ``True`` (default), only alerts with
            ``is_active = True`` are returned. Set to ``False`` to retrieve
            all historical alerts including deactivated ones.
        severity: Optional filter. Accepted values are ``"info"``,
            ``"warning"``, and ``"danger"``. When omitted, alerts of all
            severities are returned.
        db: SQLAlchemy session injected by FastAPI's dependency system.

    Returns:
        A list of ``WeatherAlertSchema`` objects ordered by ``created_at``
